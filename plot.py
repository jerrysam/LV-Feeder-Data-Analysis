...
for csv_file in csv_files:
    ...
    data['Timestamp_Description'] = data['Timestamp'].astype(str) + ' - ' + data['Description']
    data = data.set_index('Timestamp_Description')
    
    pivot_data = data.pivot_table(index='Timestamp', columns='Description', values='Value', aggfunc='first')
    ...
    output_df.to_csv(os.path.join(os.path.dirname(__file__), '../outputcsv', f'{data.iloc[1, 0]} - {data.iloc[1, 1]}.csv'), index=False)
